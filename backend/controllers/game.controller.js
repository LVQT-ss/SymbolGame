...
export const completeGame = async (req, res) => {
    ...
    try {
        ...
        if (!game_session_id || !total_time || !rounds || !Array.isArray(rounds)) {
            return res.status(400).json({
                message: 'Missing required fields: game_session_id, total_time, and rounds array are required.'
            });
        }
        ...
        // Update round details and calculate score
        let correctAnswers = 0;
        const roundUpdates = [];

        for (const round of rounds) {
            const roundDetail = gameSession.rounds.find(r => r.round_number === round.round_number);
            if (!roundDetail) {
                return res.status(400).json({
                    message: `Round ${round.round_number} not found in game session.`
                });
            }

            const isCorrect = round.user_symbol === roundDetail.correct_symbol;
        ...
        // Calculate final score (100 points per correct answer)
        const finalScore = correctAnswers * 100;

        // Update game session and round details in a transaction
        await sequelize.transaction(async (t) => {
            ...
            for (const update of roundUpdates) {
                await RoundDetail.update({
                    user_symbol: update.user_symbol,
        ...
        res.status(200).json({
            ...
            rounds: roundUpdates.map((update, index) => ({
                round_number: rounds[index].round_number,
                user_symbol: update.user_symbol,
                response_time: update.response_time,
                is_correct: update.is_correct
            }))
